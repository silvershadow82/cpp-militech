...
protected:
  ...
  const models::BBox lockBox{.x = 272.0, .y = 192.0, .w = 96.0, .h = 96.0};
  const control::TrackerRequest lockCenter{.kind = control::TrackerRequestKind::LockCenter, .hint = lockBox};
};
...
TEST_F(SyntheticCameraTest, TargetDeadAheadIsHorizontallyCentered)
{
  ...
  std::optional<models::BBox> box = cam.project(hover, {3.0, 0.0, 0.0}, 1.7, 0.5);
...
{
  SyntheticCamera cam(camera, {}, config);
  std::optional<models::BBox> box = cam.project(hover, {3.0, 1.0, 0.0}, 1.7, 0.5);
  ASSERT_TRUE(box);
...
TEST_F(SyntheticCameraTest, ObservationArrivesAfterLatency)
{
  ...
  // Run
  std::optional<models::TargetObservation> early = cam.step(at(0.0), hover, target, 0.0);
  std::optional<models::TargetObservation> stillEarly = cam.step(at(0.05), hover, target, 0.05);
  std::optional<models::TargetObservation> delivered = cam.step(at(0.1), hover, target, 0.1);
...
TEST_F(SyntheticCameraTest, LockMissWhenTargetOutsideLockBox)
{
  ...
  std::optional<models::TargetObservation> obs = cam.step(at(0.1), hover, target, 0.1);
...
TEST_F(SyntheticCameraTest, OcclusionLosesLockAndReacquireRestoresIt)
{
  ...
  std::optional<models::BBox> truth = cam.project(hover, {3.0, 0.0, 0.0}, 1.7, 0.5);
  ASSERT_TRUE(truth);
  cam.handle(lockCenter, at(0.0));

  // Run
  std::optional<models::TargetObservation> obs;
  for (int i = 0; i <= 6; ++i) {
  ...
  cam.handle({.kind = control::TrackerRequestKind::Reacquire, .hint = *truth}, at(0.3));
  for (int i = 7; i <= 20; ++i) {
    obs = cam.step(at(i * 0.05), hover, target, i * 0.05);
  }
...
TEST_F(SyntheticCameraTest, OffCenterTargetKeepsAngularSizeButNotPixelSize)
{
  ...
  SyntheticCamera cam(camera, {}, config);
  double side = 4.0 * std::cos(models::degToRad(45.0));
  std::optional<models::BBox> ahead = cam.project(hover, {4.0, 0.0, 0.0}, 1.7, 0.5);
  std::optional<models::BBox> offCenter = cam.project(hover, {side, side, 0.0}, 1.7, 0.5);
  ASSERT_TRUE(ahead);
  ASSERT_TRUE(offCenter);

  models::AttitudeHistory level;
  level.push({.t = at(0.0)});
  models::EstimatorConfig estimatorConfig{};
  estimatorConfig.emaAlpha = 1.0;
  estimatorConfig.maxAreaJump = 10.0;
  control::TargetEstimator estimator(estimatorConfig, camera, {});

  // Run
  models::TargetState first = estimator.update(
    at(0.05), level, models::TargetObservation{.tFrame = at(0.0), .box = *ahead, .ok = true, .confidence = 1.0}, std::nullopt);
  models::TargetState second = estimator.update(
    at(0.1), level, models::TargetObservation{.tFrame = at(0.05), .box = *offCenter, .ok = true, .confidence = 1.0}, std::nullopt);
...
TEST_F(SyntheticCameraTest, UnlockStopsValidObservations)
{
  ...
  cam.handle({.kind = control::TrackerRequestKind::Unlock}, at(0.05));
  std::optional<models::TargetObservation> obs;
  for (int i = 1; i <= 4; ++i) {
    obs = cam.step(at(i * 0.05), hover, target, i * 0.05);
  }
