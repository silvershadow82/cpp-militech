...
constexpr size_t TX_PENDING_LIMIT = 4096;
...
bool SerialLink::open(const char *dev)
{
  ...
  termios tio{};
  if (tcgetattr(fd, &tio) == 0) {  // справжній tty — налаштовуємо лінію
...
Frame SerialLink::drainBuffered()
{
