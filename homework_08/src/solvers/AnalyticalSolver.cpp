...
float AnalyticalSolver::payloadTimeOfFlight() {
  ...
  float b =
      static_cast<float>(-3.0 * G * pp.m2 + 3.0 * pp.d * pp.l * pp.m * speed);
  ...
  float t = static_cast<float>(
      2.0 * sqrt(-p / 3.0) * cos((phi + 4.0 * M_PI) / 3.0) - b / (3.0 * a));
...
float AnalyticalSolver::calcHDistance(float t) {
  ...
  float e3 = static_cast<float>(
      t3 *
      (6.0 * pp.d * G * pp.l * pp.m - 6.0 * pp.d2 * (pp.l2 - 1.0) * speed) /
      (36.0 * pp.m2));
  float e4 = static_cast<float>(
      t4 *
      (3.0 * pp.d3 * (pp.l2 + 1.0) * pp.l2 * speed +
       6.0 * pp.d3 * (pp.l2 + 1.0) * pp.l4 * speed -
       6.0 * pp.d2 * G * (pp.l4 + pp.l2 + 1.0) * pp.l * pp.m) /
      (36.0 * pow((pp.l2 + 1.0), 2) * pp.m3));
  float e5 = static_cast<float>(t5 *
                                (3.0 * pp.d3 * G * pp.l3 * pp.m -
                                 3.0 * pp.d4 * pp.l2 * (pp.l2 + 1.0) * speed) /
...
void AnalyticalSolver::init(const DroneConfig &droneConfig,
                            const PayloadParams &payloadParams) {
  this->droneConfig = droneConfig;
  this->pp = payloadParams;
}

BallisticResult AnalyticalSolver::solve(Coord dronePos, Coord targetPos,
                                        float droneAngle) {
  ...
  if (h + droneConfig.accelPath > distanceToTarget) {
    if (fabs(distanceToTarget) < 1e-6) {
      ...
      newDronePos = targetPos - (targetPos - dronePos) *
                                    (h + droneConfig.accelPath) /
                                    distanceToTarget;
  ...
  Coord dir = {static_cast<float>(cos(droneAngle)),
               static_cast<float>(sin(droneAngle))};

  BallisticResult result{.ok = true,
                         .dropPoint =
                             newDronePos + (targetPos - newDronePos) * ratio,
                         .aimPoint = newDronePos + dir * h,
                         .payloadDropTime = t};

  DEBUG("Ballistic Result: dropPoint=(" << result.dropPoint.x << ","
                                        << result.dropPoint.y << ")");
