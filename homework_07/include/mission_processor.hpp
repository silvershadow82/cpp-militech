...
public:
  ...
  bool init(ConfigSource configSource, const std::string& dataFolder)
  {
    ...
    switch (configSource) {
      case ConfigSource::FILE:
        configLoader = ConfigLoaderFactory::createLoader(LoaderType::FILE, dataFolder);
        solver = BallisticSolverFactory::createSolver(SolverType::ANALYTICAL);
        targetProvider = TargetProviderFactory::createProvider(ProviderType::JSON, dataFolder + "/targets.json");
        break;
      default:
        break;
    }
  ...
  bool hasNext() { return this->state.step < MAX_STEPS; };
  bool step()
  {
  ...
  void reset() { initState(this->configLoader->getConfig(), this->targetProvider->getTargetCount()); };
  void changeSolver(IBallisticSolver* solver) { this->solver = solver; }
  int totalSteps() { return this->state.step; }
  void printStats()
  {
