...
struct ScenarioExpect {
  ...
  std::optional<std::vector<models::State>> states{};  // exact state sequence from engage on
  std::vector<models::State> statesInOrder{};          // must appear in this order, other states may be between
  std::optional<models::State> finalState{};
