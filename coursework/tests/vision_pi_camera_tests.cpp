...
{
  follow::providers::PiCameraConfig config{};
  ...
  std::string pipeline = follow::providers::piCameraPipeline(config);
...
{
  std::string pipeline = follow::providers::piCameraPipeline(follow::providers::PiCameraConfig{});
...
// A single flip mirrors the image and is rejected by the config loader (vision.hflip/vflip must match) --
// see FileConfigLoader.cpp and ConfigJson.AsymmetricFlipIsRejected. That validation lives in the config
...
TEST(PiCameraFailureBudget, TolerancesOneAndAHalfSecondsOfConsecutiveMisses)
{
  ...
  EXPECT_EQ(follow::providers::piCameraFailureBudget(20), 30);
  EXPECT_EQ(follow::providers::piCameraFailureBudget(30), 45);
  EXPECT_EQ(follow::providers::piCameraFailureBudget(15), 23);  // ceil(22.5)
}
...
TEST(PiCameraFailureBudget, IsAtLeastOneFrameForANonsenseRate)
{
  ...
  EXPECT_EQ(follow::providers::piCameraFailureBudget(0), 1);
  EXPECT_EQ(follow::providers::piCameraFailureBudget(-5), 1);
}
